...
export default function DogForm({ dog, reset, getDogs }) {
  ...
  useEffect(() => {
    fetch('/api/dogs/breeds')
      .then(res => res.json())
      .then(breeds => setBreeds(breeds.sort())) // Sorting breeds
  ...
  const postDog = () => {
    ...
    fetch('/api/dogs', {
      ...
      .then(res => {
        ...
        return res.json(); // Ensure this line to parse the response
      })
      .then(() => {
...
const putDog = () => {
  ...
  fetch(`/api/dogs/${dog.id}`, {
    ...
    .then(res => {
      if (!res.ok) {
        throw new Error('Problem PUTting dog');
      }
      return res.json(); // Ensure this line to parse the response
    })
    .then(() => {
